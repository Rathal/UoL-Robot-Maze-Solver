...
class Proximity:
    ...
    def laser_cb(self, laser_msg):
        ...
        # if any(0 < r < 0.2 for r in laser_msg.ranges) or any(np.isnan(r) for r in laser_msg.ranges):
        #     print 'EMERGENCYEMERGENCYEMERGENCYEMERGENCY'
        #     self.emegency = True
        ...
        if self.emegency == False:

            if any(0 < r < 0.1 for r in laser_msg.ranges):
                self.t.linear.x = -1
